...
<head>
    ...
    <title>Edit Petugas - HyperRack</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .submit-row { display: flex; justify-content: space-between; align-items: center; margin-top: 20px; border-top: 1px solid #eee; padding-top: 20px; }
        ...
        @media (max-width: 600px) {
            ...
            .submit-row { flex-direction: column; gap: 15px; }
            .btn-simpan { width: 100%; text-align: center; }
        }
    </style>
</head>
<body>
    ...
    <main class="main-content">
        ...
        <div class="content-body">
            <div class="header-row">
                <a href="{{ route('admin.petugas') }}" class="btn-back">←</a>
                <h2 style="font-size: 24px; font-weight: 700;">Edit Petugas</h2>
            </div>
            ...
            <div class="form-card">
                <form action="{{ route('admin.petugas.update', $petugas->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        <label>Status</label>
                        <select name="status">
                            <option value="aktif" {{ ($petugas->status ?? 'aktif') == 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="nonaktif" {{ ($petugas->status ?? '') == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>

                    <div class="submit-row">
                        <a href="{{ route('admin.petugas') }}" class="btn-batal">Kembali ke Daftar</a>
